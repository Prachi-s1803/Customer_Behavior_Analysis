...
select customer_id , purchase_amount, (select avg(purchase_amount) from customer )
from customer
where discount_applied= 'Yes' and purchase_amount > (select avg(purchase_amount) from customer );

-- Q3: which are the top 5 products with the highest average review rating?
select item_purchased ,round(avg(review_rating):: numeric,2) as avg_rev_rating_byitem
from customer
group by item_purchased
order by avg(review_rating) desc
limit 5;
-- as it shows the error of double precision. so, we type cast it to numeric.
